    ...
    def test_step(self, batch, batch_idx):
      ...
      #  print(batch['graph_tree'].mesh_name[0])
        if global_config.mode == "test_visualize":
            ...
            ################
            # my addidtion for extraction the predicted Laplacian matrix

            from scipy.sparse import coo_matrix, diags, save_npz
            import os

            # graph at level 0
            G0        = batch['graph_tree'].treedict[0]
            edge_idx  = G0.edge_index             # [2, E] LongTensor
            weights   = result["edge_weight_predicted"].squeeze().cpu().numpy()  # [E]

            # build symmetric adjacency A = (W + Wᵀ)/2
            i, j = edge_idx[0].cpu().numpy(), edge_idx[1].cpu().numpy()
            n = G0.x.shape[0]
            A = coo_matrix((weights, (i, j)), shape=(n, n))
            A     = 0.5 * (A + A.T)

            # degree & Laplacian L = D – A
            deg   = np.array(A.sum(axis=1)).flatten()
            D     = diags(deg)
            L_ours = D - A

            # save to disk
            out_dir = "out/predicted_L"
            os.makedirs(out_dir, exist_ok=True)
            raw_name  = os.path.basename(batch['graph_tree'].mesh_name[0])
            print("#########################################################")
            print(raw_name)
            print("#########################################################")
            base_name = os.path.splitext(raw_name)[0]
            filename  = os.path.join(out_dir, base_name + ".npz")
            save_npz(filename, L_ours)
            console.log(f"\nSaved learned Laplacian → {filename}", style="green")
            ################
